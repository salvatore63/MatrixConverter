...
)
{
	...
	if (vol <= 9 && p[1] == ':') {	/* Found a drive number, get and strip it */
                //if (vol == 0) STM32F100PinToggle( GPIOC,GPIO_Pin_8 );                            // Salvatore
	...
	fs = FatFs[vol];			/* Return pointer to the corresponding file system object */
        *rfs = fs ;
	if (!fs) return FR_NOT_ENABLED;         // Is the file system object available?  Salvatore esclude
...
)
{
	...
	res = chk_mounted(&path, &dj.fs, 0);//res = chk_mounted(&path, &dj.fs, (BYTE)(mode & (FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)));
#else
	mode &= FA_READ;
